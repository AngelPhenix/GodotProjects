...
func set_selection(new_index):
	if new_index >= 0 && new_index < len(positions):
		...
		position = Vector2(selected_node.rect_position.x - (texture.get_width()+25), selected_node.rect_position.y + selected_node.rect_size.y/2)
...
func _process(delta):
	...
	if Input.is_action_just_pressed("ui_down") || Input.is_action_just_pressed("down"):
		set_selection(index + 1)
	if Input.is_action_just_pressed("ui_select") || Input.is_action_just_pressed("jump"):
